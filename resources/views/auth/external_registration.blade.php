...
<div class="container">
    <div class="row justify-content-center">
        ...
        <div class="col-md-6 mt-5  register">
            <div class="card box-shadow">
                ...
                <div class="card-header bg-white access">
                    <h3><i class="fa fa-book"></i> Register <span class="register_as"></span></h3>
                ...
                <div class="card-body">
                    ...
                    <div class="Student">
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="hidden" value="{{$exSchId}}" id="exSchId"></input>
                            </div>
                        </div>
                    ...
                    <div class="form-group row dflex">
...
$(document).on("click", ".login_facebook", function(){
    ...
    if(uType){
        $.ajax({
          ...
          method: "get",
          beforeSend: function(){
            $(this).removeClass("btn-primary").addClass("bg-warning").html("<i class='fa fa-spinner fa-spin'></i>")
          },
...
$(document).on("click", ".col-md-3 .box-shadow", function(){
    $(".dnone").hide();
    $(this).parents(".uRegister").find(".bg-green").removeClass("bg-green").addClass("bg-warning");
    $(this).parents(".uRegister").find(".cUrrent").removeClass("cUrrent");
    $(this).removeClass("bg-warning").addClass("bg-green");
...
$(document).on("click", ".user_res", function(){
   ...
   // return true;
    ...
    {
    alert('Wrong Password...! minimum 6 characters which contain at least one numeric digit, one uppercase and one lowercase letter');
    return false;
    }

   if(uType == 'External Student'){
        var StudentFirstName = $("#StudentFirstName").val();
        var StudentLastName = $("#StudentLastName").val();
        var StudentID = $("#StudentID").val();
        var StdClass = $("#StdClass").val();
        var exSchId = $("#exSchId").val();;
        if(StudentFirstName == ''){
    ...
    $.ajax({
        url: "{{ url('external-Registration') }}",
        method: "post",
        data: data,
        beforeSend: function(){
            ts.removeClass("bg-navy").addClass("bg-warning").html("<i class='fa fa-spinner fa-spin'></i>")
        },
        success: function(rsp){
            if(rsp.error == false){
               console.log(rsp.data.user.usertype);
               console.log('ExID');
               console.log(rsp.data['exSchId']);
                $(".sErr").html("<div class='alert alert-success'>"+rsp.msg+"</div>");
                $("input").val("");
                ts.removeClass("bg-warning").addClass("bg-navy").html("<i class='fa fa-save'></i> Submit");
                setTimeout(function(){
                    if(rsp.data.user.usertype == 'External Student' || rsp.data.user.usertype == 'Authority' || rsp.data.user.usertype == 'Parent' || rsp.data.user.usertype == 'Teacher'){
                    window.location = "{{ url('external-home-') }}"+rsp.data['exSchId'];
                    }
